...
AI-powered image generation system with multiple pipelines using OpenAI's DALL-E 3 and GPT-4.1 APIs. Supports batch processing with nested folder structures and configurable styling via JSON configuration files.
...
1. **V1 Vision Pipeline** (`src/openai_image_generator.py`)
   - Flow: Input Image → GPT-4V Analysis → Wrap in Style Template → DALL-E 3 Generation
...
2. **V2 Enhanced Pipeline** (`src/openai_image_generator_v2_simple.py`)
   - Flow: Image → GPT-4.1 Analysis & Generation (single API call)
...
4. **Animation Creator** (`scripts/create_evolution_animation.py`)
